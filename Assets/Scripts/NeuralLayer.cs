...
public class NeuralLayer : MonoBehaviour
{
    ...
    {
            return x < -45.0 ? 0.0 : x > 45.0 ? 1.0 : 1.0 / (1.0 + System.Math.Exp(-x));
    }
    ...
    public ActivationFunction NeuronActivationFunction = sigmoid;//MathHelper.SigmoidFunction;
    ...
    public void SetWeights(double[] weights)
    {
        ...
        if (weights.Length != this.Weights.Length)
            throw new System.ArgumentException("Input weights do not match layer weight count.");
    ...
    public double[] ProcessInputs(double[] inputs)
    {
        ...
        if (inputs.Length != NeuronCount)
            throw new System.ArgumentException("Given xValues do not match layer input count.");
    ...
    {
        double range = System.Math.Abs(minValue - maxValue);
